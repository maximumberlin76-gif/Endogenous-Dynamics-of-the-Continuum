...
def build_protocol(
    ...
    **config_changes: object,
    ...
    return EDKMarnovRetentionCollapseProtocol(
        build_engine(seed, num_domains),
        build_config(**config_changes),
    )


class ConfigurationTests(unittest.TestCase):
    def test_valid_configuration(self) -> None:
        build_config().validate(18.0)

    def test_invalid_configuration_values(self) -> None:
        cases = (
            (
                {"formation_maximum_tacts": 0},
                "formation_maximum_tacts must be at least 1",
            ),
            (
                {
                ...
                "formation_confirmation_tacts exceeds formation_maximum_tacts",
            ),
            (
            ...
            (
                ...
                "retained_boundary_tolerance must be finite and non-negative",
            ),
            (
                {"delay_base_tau_0": 0.0},
                "delay_base_tau_0 must be a positive finite value",
            ),
            (
                {
            ...
            (
                {"coupling_floor": 18.1},
                "coupling_floor exceeds the initial coupling strength",
            ),
            (
        ...
        for changes, message in cases:
            with self.subTest(changes=changes):
                with self.assertRaisesRegex(ValueError, message):
                    build_config(**changes).validate(18.0)
...
class RuntimeTests(unittest.TestCase):
    ...
    def test_initial_metrics(self) -> None:
        ...
        self.assertEqual(metrics["protocol_state"], self.protocol.INITIALIZED)
        self.assertEqual(metrics["final_status"], self.protocol.INITIALIZED)
        self.assertFalse(metrics["attractor_formed"])
        self.assertFalse(metrics["phase_node_unlocked"])
        self.assertFalse(metrics["collapse_detected"])
        self.assertGreaterEqual(metrics["C_proxy_t"], 0.0)
        self.assertLessEqual(metrics["C_proxy_t"], 1.0)
        self.assertAlmostEqual(
            metrics["retention_margin"],
            metrics["C_proxy_t"] - metrics["external_pressure_P_ext"],
        )

    def test_invalid_runtime_inputs(self) -> None:
        for dt in (0.0, -0.01, float("inf"), float("nan")):
            with self.subTest(dt=dt):
                with self.assertRaisesRegex(
                    ValueError,
                    "dt must be a positive finite value",
                ):
                    self.protocol.run_protocol(dt, 1.0, 0.0)

        forcing_cases = (
            (float("nan"), 0.0, "Forcing density must be finite"),
            (1.0, float("inf"), "Forcing phase must be finite"),
        )
        for density, phase, message in forcing_cases:
            with self.subTest(density=density, phase=phase):
                with self.assertRaisesRegex(ValueError, message):
                    self.protocol.run_protocol(0.01, density, phase)

        for pressure in (-0.01, 1.01):
            with self.subTest(pressure=pressure):
                with self.assertRaisesRegex(
                    ValueError,
                    "external_pressure must be within",
                ):
                    self.protocol._validate_pressure(pressure)

        with self.assertRaisesRegex(
            ValueError,
            "external_pressure must be finite",
        ):
            self.protocol._validate_pressure(float("nan"))

    def test_retention_regime_classification(self) -> None:
        tolerance = self.protocol.config.retained_boundary_tolerance
        self.assertEqual(
            self.protocol._classify_retention_regime(tolerance + 0.01),
            self.protocol.EDS_RETAINED,
        )
        self.assertEqual(
            self.protocol._classify_retention_regime(tolerance),
    ...
    def test_delay_and_critical_exposure(self) -> None:
        tau_low = self.protocol._calculate_delay_tau(0.01)
        tau_high = self.protocol._calculate_delay_tau(0.50)
        self.assertGreater(tau_low, tau_high)
        self.assertGreaterEqual(
            tau_high,
            self.protocol.config.minimum_delay_tau,
        )

        self.protocol.pressure_excess = 0.5
        self.protocol.instantaneous_delay_tau = 0.25
        self.protocol._accumulate_critical_exposure(0.1)
        self.assertAlmostEqual(self.protocol.critical_exposure, 0.4)

        self.protocol.pressure_excess = 0.0
        self.protocol._accumulate_critical_exposure(0.2)
        self.assertAlmostEqual(self.protocol.critical_exposure, 0.35)

    def test_pressure_schedules(self) -> None:
        step = build_protocol(pressure_schedule="step")
        self.assertEqual(
            step._pressure_for_loading_tact(1, 0.1, None),
        ...
        self.assertAlmostEqual(
            linear._pressure_for_loading_tact(2, 0.1, None),
            0.2,
        )
        ...
        smooth_value = smooth._pressure_for_loading_tact(3, 0.1, None)
        self.assertGreater(smooth_value, 0.1)
        self.assertLess(smooth_value, 0.9)

        external = build_protocol(pressure_schedule="external_sequence")
        self.assertEqual(
            external._pressure_for_loading_tact(5, 0.1, (0.2, 0.4)),
            0.4,
        )
        with self.assertRaisesRegex(
            ValueError,
            "A non-empty external pressure sequence is required",
        ):
            external._pressure_for_loading_tact(1, 0.1, ())

    def test_field_snapshot(self) -> None:
        initial_base = self.protocol.base_natural_frequencies.copy()
        snapshot = self.protocol.export_field_snapshot()
        fields = (
            "phases",
            "amplitudes",
            "natural_frequencies",
        ...
        for key in fields:
            values = np.asarray(snapshot[key])
            self.assertEqual(values.shape, (self.protocol.engine.N,))
            self.assertTrue(
                np.all(np.isfinite(values)),
                msg=f"Non-finite values in {key}",
            )
    ...
    def test_full_protocol(self) -> None:
        summary = self.protocol.run_protocol(
            dt=0.01,
        ...
        self.assertTrue(summary["attractor_formed"])
        self.assertTrue(summary["attractor_verified"])
        self.assertTrue(summary["phase_node_unlocked"])
        self.assertTrue(summary["collapse_detected"])
        self.assertGreater(summary["history_length"], 0)

        events = [item["event"] for item in summary["transition_events"]]
        for event in (
            "ATTRACTOR_FORMED",
            "ATTRACTOR_VERIFIED",
            "PHASE_NODE_UNLOCKED",
        ...
        self.assertLess(summary["formation_tact"], summary["unlock_tact"])
        self.assertLess(summary["unlock_tact"], summary["collapse_tact"])
        ...
        for key in (
            ...
            self.assertTrue(
                math.isfinite(float(summary["final_metrics"][key])),
                msg=f"Non-finite metric: {key}",
            )
        ...
        np.testing.assert_array_equal(
            ...
            self.protocol.engine._to_host(
                self.protocol.engine.natural_frequencies
            ),
        )

    def test_same_seed_reproduces_transition_sequence(self) -> None:
        protocol_a = build_protocol(seed=7)
        protocol_b = build_protocol(seed=7)
        summary_a = protocol_a.run_protocol(0.01, 1.5, 0.0)
        summary_b = protocol_b.run_protocol(0.01, 1.5, 0.0)
        for key in (
            "final_status",
            "formation_tact",
            "unlock_tact",
            ...
            self.assertEqual(summary_a[key], summary_b[key])


class LoggerTests(unittest.TestCase):
    def test_logger_outputs(self) -> None:
        protocol = build_protocol(seed=13)
        protocol._advance_engine(1.0, 0.0, 0.01)
        protocol._refresh_protocol_observables(
            protocol.config.pressure_hold
        )

        with tempfile.TemporaryDirectory() as directory:
            logger = EDKMarnovProtocolLogger(directory)
            metrics_path, field_path = logger.log_step(
                protocol,
                include_field=True,
            )
            summary_path = logger.log_summary(protocol)

            self.assertTrue(metrics_path.is_file())
            self.assertIsNotNone(field_path)
            ...
            self.assertEqual(
                payload["protocol_class"],
                "EDKMarnovRetentionCollapseProtocol",
            )
            self.assertIn("metrics", payload)
            self.assertIn("protocol_configuration", payload)
