...
C^3 is the cubic nonlinear saturation, compression, and delay of the phase-coherent configuration.

C^3 = |Psi_coh|^3

C^3 is a spatial field participating in the formation of the phase-transition window geometry Omega(t), the local manifested-mass density, and the damping of the local vector exchange field J.
...
The toroidal geometry of C^3 is projected into the center of the computational grid.
...
C^3 affects the geometry and local structure of the phase-transition window.

Retention is determined by the general endogenous structural coherence C(t), not by the field C^3.
...
mass_field[eds_mask] = grad_rho_magnitude[eds_mask] · C^3[eds_mask] / c^2

T_int is preserved as an independent dynamic interface state and is not replaced by C^3, C(t), or M(t).
...
The second term represents damping of the local exchange field through cubic nonlinear saturation, compression, and delay C^3.
...
T_target = interface_operator(C_t, C^3, Omega(t), D_n_next)
...
3. Formation of the independent field C^3.
...
1. The cubic nonlinear saturation, compression, and delay field C^3.
...
C^3, C(t), J, and J_flux are displayed as distinct parameters.
...
C^3 remains the cubic nonlinear saturation, compression, and delay field.
...
C^3 — кубическое нелинейное насыщение, сжатие и задержка фазово-когерентной конфигурации.

C^3 = |Psi_coh|^3

C^3 является пространственным полем и участвует в формировании геометрии окна фазового перехода Omega(t), локальной плотности манифестированной массы и торможении локального векторного поля обмена J.
...
Тороидальная геометрия C^3 проецируется в центр расчётной сетки.
...
C^3 влияет на геометрию и локальную структуру окна фазового перехода.

Удержание определяется общей эндогенной структурной когерентностью C(t), а не полем C^3.
...
T_int сохраняется как самостоятельное динамическое состояние интерфейса и не подменяется через C^3, C(t) или M(t).
...
Второе слагаемое описывает торможение локального поля обмена через кубическое нелинейное насыщение, сжатие и задержку C^3.
...
3. Формирование самостоятельного поля C^3.
...
C^3, C(t), J и J_flux отображаются как разные параметры.
...
C^3 остаётся полем кубического нелинейного насыщения, сжатия и задержки.
