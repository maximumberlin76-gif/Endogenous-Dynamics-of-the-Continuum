...
## EN — 4A. Generalized Critical Ramp-Scaling Closure

For the local normal form
...
For `n = 3`, the amplitude scale is `g^(-1/2) v_eff^(1/4)`, while the temporal exponent remains `-1/2`. The geometric closure `V_coh,d ∝ C^d`, the identification `n = d`, and the symmetry closure under `C -> -C` are separate from the exponent balance. The complete proof is fixed in the [Generalized Ramp-Scaling Lemma](10_generalized_ramp_scaling_lemma_EN_RU.md).
...
N_C > 0 is the normalization factor of the folded-state intensity, and C := [(1 / N_C) Tr(Psi_coh^dagger Psi_coh)]^(1/3).
...
## RU — 4A. Обобщённое замыкание критического рампового масштабирования

Для локальной нормальной формы
...
Для `n = 3` амплитудный масштаб равен `g^(-1/2) v_eff^(1/4)`, тогда как временной показатель сохраняется равным `-1/2`. Геометрическое замыкание `V_coh,d ∝ C^d`, отождествление `n = d` и симметрийное замыкание относительно `C -> -C` отделены от баланса показателей. Полное доказательство зафиксировано в [Обобщённой лемме рампового масштабирования](10_generalized_ramp_scaling_lemma_EN_RU.md).
...
N_C > 0 — нормировочный коэффициент интенсивности свёрнутого состояния, а C := [(1 / N_C) Tr(Psi_coh^dagger Psi_coh)]^(1/3).
