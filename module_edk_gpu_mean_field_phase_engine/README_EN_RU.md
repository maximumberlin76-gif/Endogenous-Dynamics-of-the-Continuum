...
- a complete plasma-physics solver;
- a magnetohydrodynamic solver;
- a molecular-dynamics solver;
- a mass-materialization model;
- a complete calculation of C(t);
- a complete calculation of T_int;
- a complete reconstruction of J_flux.
...
Сам по себе модуль не является:

- полным решателем физики плазмы;
- магнитогидродинамическим решателем;
- решателем молекулярной динамики;
- моделью материализации массы;
- полным расчётом C(t);
- полным расчётом T_int;
- полным восстановлением J_flux.
